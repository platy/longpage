use std::{future::Future, ops::{Add, Range}};
...
pub fn next_request_for_view<T>(data: &SparseVec<T>, in_view: Range<usize>) -> Option<Range<usize>> {
    if in_view.len() == 0 {
        return None
    }
    let extra_load = in_view.len() / 2;
    let should_load = in_view.start.checked_sub(extra_load).unwrap_or(0) .. (in_view.end + extra_load).min(data.len());
    Some(should_load)
}
